...
class ProductsController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id != 1){
            $products = Product::where('store_id', Auth::user()->store->id)->paginate(6);
        }
        $number_of_products = Product::distinct('id')->count('id');
    ...
    public function store(Request $request)
    {
        ...
        $product_code = 'TWT-' . $this->better_uniqid(8);
    ...
    public function show($code)
    {
        $product = Product::with('store.user')
            ...
            ->where('code', $code)
            ->groupBy('code', 'title', 'price', 'status', 'gender', 'description')
            ->first();

        $productForImages = Product::where('code', $code)->first();
        $images = $productForImages->images;
        // $images = Product::find($product->ids[0])->images()->get();
    ...
    public function edit(Product $product)
    {
        ...
        $Categories = Category::all();
        return view('admin.apps-ecommerce-product-edit',[
    ...
    protected function better_uniqid($length = 12) {
    ...
    public function update(Request $request)
    {
        ...
        {
            $quantity = $request->input("qualityInput{$productId}");
            $color = $request->input("selectColor{$productId}");
            $size = $request->input("selectSize{$productId}");
        ...
        $product_id_for_image = Product::where('code', $request->code)->pluck('id')->first();
    ...
    {
        $product = Product::find($request->this_id);
