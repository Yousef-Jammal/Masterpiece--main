                            ...
                            <div class="card-body">
                                <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                                    <div class="p-4 rounded-md lg:col-span-2 bg-slate-100 dark:bg-zink-600">
                                        <img src="{{ asset('images/product/' . $wishlist->product->images->first()->name ) }}" alt="">
                                    </div><!--end col-->
                                    <div class="flex flex-col lg:col-span-4">
                                        <div>
                                            <h5 class="mb-1 text-16"><a href="apps-ecommerce-product-overview.html">{{ $wishlist->product->title }}</a></h5>
                                            <p class="mb-2 text-slate-500 dark:text-zink-200"><a href="#!">{{ $wishlist->product->category->name }}</a></p>
                                            @php
                                                // $colorName = getColorName($wishlist->product->color);
                                            @endphp
                                            <p class="mb-1 text-slate-500 dark:text-zink-200">Color:
                                                 <span class="text-slate-800 dark:text-zink-50">{{ getColorName($wishlist->product->color) }}</span>
                                                 {{-- <div class="flex flex-wrap items-center gap-2"> --}}
                                                    {{-- @foreach ($wishlist->$product->colors as $color) --}}
                                                        {{-- <input style="background-color: {{ $wishlist->product->color }} ; border-color:{{ $wishlist->product->color }}" type="radio" value="{{ $wishlist->product->color }}" name="selectColor" id="{{ $wishlist->product->color }}" class="inline-block align-middle border rounded-sm appearance-none cursor-pointer size-5 bg-sky-500 border-sky-500 checked:bg-sky-500 checked:border-sky-500 disabled:opacity-75 disabled:cursor-default" > --}}
                                                        {{-- <span style="background-color: {{ $wishlist->product->color }} ; border-color:{{ $wishlist->product->color }}" value="{{ $wishlist->product->color }}" name="selectColor" id="{{ $wishlist->product->color }}" class="inline-block align-middle border rounded-sm appearance-none cursor-pointer size-5 bg-sky-500 border-sky-500 checked:bg-sky-500 checked:border-sky-500 disabled:opacity-75 disabled:cursor-default"></span> --}}
                                                    {{-- @endforeach --}}
                                                {{-- </div> --}}
                                            </p>
                                            <p class="mb-3 text-slate-500 dark:text-zink-200">Size: <span class="text-slate-800 dark:text-zink-50">{{ $wishlist->product->size }}</span></p>
                                        </div>
                                        <div class="flex items-center gap-2 mt-auto">
                                            {{-- <div class="inline-flex p-2 text-center border rounded input-step border-slate-200 dark:border-zink-500">
                                                ...
                                                <input id="cartCount" data-id="{{ $wishlist->id }}" type="number" class="text-center ltr:pl-2 rtl:pr-2 w-15 h-7 products-quantity dark:bg-zink-700 focus:shadow-none" value="1" min="0" max="100" readonly>
                                            ...
                                            {{-- <div class="inline-flex p-2 text-center border rounded input-step border-slate-200 dark:border-zink-500">
                                                ...
                                                <input id="cartCount" data-id="{{ $wishlist->id }}" type="number" class="products-quantity w-15 h-7 text-center" value="{{ $wishlist->quantity }}" min="1" max="100" readonly>
                                            ...
                                            <button  data-id="{{ $wishlist->id }}" id="deleteRecord" type="button" class="flex items-center justify-center size-[37.5px] p-0 text-red-500 bg-red-100 btn hover:text-white hover:bg-red-600 focus:text-white focus:bg-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:ring active:ring-red-100 dark:bg-red-500/20 dark:text-red-500 dark:hover:bg-red-500 dark:hover:text-white dark:focus:bg-red-500 dark:focus:text-white dark:active:bg-red-500 dark:active:text-white dark:ring-red-400/20 remove-button"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                                        </div>
                                    </div><!--end col-->
                                    <div class="flex justify-between w-full lg:flex-col lg:col-end-13 lg:col-span-2">
                                        <div class="mb-auto ltr:lg:text-right rtl:lg:text-left">
                                            <h6 class="text-16 products-price">$<span>{{ $wishlist->product->price }}</span>
                                                {{-- <small class="font-normal line-through text-slate-500 dark:text-zink-200">$299.99</small> --}}
                                            </h6>
                                        </div>
                                        <h6 class="mt-auto text-16 ltr:lg:text-right rtl:lg:text-left">$<span class="products-line-price">{{ $wishlist->product->price }}</span></h6>
                                    </div><!--end col-->
                                </div><!--end grid-->
                            </div>
                        </div><!--end card-->
...
$(document).ready(function() {
    ...
    $('#deleteRecord').on('click', function() {
        ...
        var url = `{{ route('delete_wishlist', '') }}/${recordId}`;
