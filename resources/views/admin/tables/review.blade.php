                                    ...
                                    <table class="w-full border-separate table-custom border-spacing-y-1 whitespace-nowrap">
                                        <thead class="text-left">
                                            <tr class="relative rounded-md bg-slate-100 dark:bg-zink-600 after:absolute ltr:after:border-l-2 rtl:after:border-r-2 ltr:after:left-0 rtl:after:right-0 after:top-0 after:bottom-0 after:border-transparent [&.active]:after:border-custom-500 [&.active]:bg-slate-100 dark:[&.active]:bg-zink-600">
                                                ...
                                                {{-- <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort">Revinew ID</th> --}}
                                                <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort">User</th>
                                                {{-- <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort">Arabic Name</th> --}}
                                                {{-- <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort">Location</th> --}}
                                                <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort" >Comment</th>
                                                <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort">rating</th>
                                                <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold sort">Product Code</th>
                                        ...
                                        <tbody class="list">
                                            ...
                                            <tr class="relative rounded-md after:absolute ltr:after:border-l-2 rtl:after:border-r-2 ltr:after:left-0 rtl:after:right-0 after:top-0 after:bottom-0 after:border-transparent [&.active]:after:border-custom-500 [&.active]:bg-slate-100 dark:[&.active]:bg-zink-600">
                                                ...
                                                <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                                    <div class="flex items-center gap-2">
                                                        {{-- <div class="flex items-center justify-center font-medium rounded-full size-10 shrink-0 bg-slate-200 text-slate-800 dark:text-zink-50 dark:bg-zink-600">
                                                            <img src="{{ asset('images/users/user-dummy-img.jpg') }}" alt="" class="h-10 rounded-full">
                                                        </div> --}}
                                                        <div class="grow">
                                                            <h6 class="mb-1"><a href="#!" class="name">{{ $review->user->name }}</a></h6>
                                                ...
                                                <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 email">{{ $review->review }}</td>
                                                <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 phone-number">{{ $review->rating }}</td>
                                                <td class="px-3.5 py-2.5">
                                                    <a href="#!" class="transition-all duration-150 ease-linear product_code text-custom-500 hover:text-custom-600">
                                                        #{{ $review->product->code }}
                                                    </a>
                                                </td>
                                                <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 joining-date">{{ $review->created_at }}</td>
                                                ...
                                                <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                                    <div class="relative dropdown">
                                                        ...
                                                        <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="usersAction1">
                                                            <li>
                                                                <a data-modal-target="editReviewModal" data-review-info='{{ json_encode($review) }}'  class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="#!"><i data-lucide="file-edit" class="inline-block size-3 ltr:mr-1 rtl:ml-1"></i> <span class="align-middle">Edit</span></a>
                                                            </li>
                                                            <li>
                                                                <a data-modal-target="deleteModal" data-this-id="{{ $review->id }}" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="#!"><i data-lucide="trash-2" class="inline-block size-3 ltr:mr-1 rtl:ml-1"></i> <span class="align-middle">Delete</span></a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </td>
...
<div id="addReviewModal" modal-center class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
            <h5 class="text-16">Add Review</h5>
            <button  data-modal-close="addReviewModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x" class="size-5"></i></button>
        </div>
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form action="{{ route('amdin_reviews_create') }}" method="POST" class="customForm" >
                @csrf
                <!-- Arabic Name Field as Select Dropdown -->
                <div class="mb-3">
                    <label for="user_id" class="inline-block mb-2 text-base font-medium">Select User</label>
                    ...
                    <div class="text-red-500 text-sm mt-1 hidden user_id_error">You should select a user</div>
                    @if ($errors->has('user_id'))
                        <div class="text-red-500 text-sm mt-1">{{ $errors->first('user_id') }}</div>
                    @endif
                </div>

                <!-- Product Name Field as Select Dropdown -->
                <div class="mb-3">
                    <label for="product_ids" class="inline-block mb-2 text-base font-medium">Select Product</label>
                    <select id="product_ids" name="product_ids" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                        <option value="" disabled selected>Select a product</option>
                        @foreach ($products as $product)
                        <option value="{{ json_encode($product['ids']) }}" {{ old('product_ids') == json_encode($product['ids']) ? 'selected' : '' }}>
                            {{ $product['title'] }}
                            </option>
                        @endforeach
                    </select>
                    <div class="text-red-500 text-sm mt-1 hidden product_error">You should select a product</div>
                    @if ($errors->has('product_ids'))
                        <div class="text-red-500 text-sm mt-1">{{ $errors->first('product_ids') }}</div>
                    @endif
                </div>

                <div class="mb-3">
                    <label for="rate" class="inline-block mb-2 text-base font-medium">Rating</label>
                    <select id="rate" name="rate" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                        <option value="" disabled selected>Select Rating</option>
                        <option value="5" {{ old('rate') == json_encode($product['ids']) ? 'selected' : '' }} >5 Star</option>
                        <option value="4" {{ old('rate') == json_encode($product['ids']) ? 'selected' : '' }} >4 Star</option>
                        <option value="3" {{ old('rate') == json_encode($product['ids']) ? 'selected' : '' }} >3 Star</option>
                        <option value="2" {{ old('rate') == json_encode($product['ids']) ? 'selected' : '' }} >2 Star</option>
                        <option value="1" {{ old('rate') == json_encode($product['ids']) ? 'selected' : '' }} >1 Star</option>
                    </select>
                    <div class="text-red-500 text-sm mt-1 hidden rate_error">You should select a rate</div>
                    @if ($errors->has('rate'))
                        <div class="text-red-500 text-sm mt-1">{{ $errors->first('rate') }}</div>
                    @endif
                </div>

                <!-- Review Number Field -->
                <div class="mb-3">
                    <label for="textArea" class="inline-block mb-2 text-base font-medium">Review</label>
                    <textarea name="comment" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" id="textArea" placeholder="Enter Review" rows="4"></textarea>
                    <div class="text-red-500 text-sm mt-1 hidden review_error">You should select a product</div>
                    @if ($errors->has('review'))
                        <div class="text-red-500 text-sm mt-1">{{ $errors->first('review') }}</div>
                    @endif
                </div>

                <div class="flex justify-end gap-2 mt-4">
                    <button type="reset" data-modal-close="addReviewModal" class="text-red-500 transition-all duration-200 ease-linear bg-white border-white btn hover:text-red-600 focus:text-red-600 active:text-red-600 dark:bg-zink-500 dark:border-zink-500">Cancel</button>
...
<div id="editReviewModal" modal-center class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
            <h5 class="text-16">Edit Review</h5>
            <button data-modal-close="editReviewModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x" class="size-5"></i></button>
        </div>
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form action="{{ route('amdin_reviews_update') }}" method="POST" class="customForm1">
                @csrf
                @method('PUT')
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <button type="reset" data-modal-close="editReviewModal" class="text-red-500 transition-all duration-200 ease-linear bg-white border-white btn hover:text-red-600 focus:text-red-600 active:text-red-600 dark:bg-zink-500 dark:border-zink-500">Cancel</button>
    ...
    <div class="w-screen md:w-[25rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto px-6 py-8">
            ...
            <div class="mt-5 text-center">
                <form action="{{ route('amdin_reviews_delete') }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <input type="hidden" name="this_id">
