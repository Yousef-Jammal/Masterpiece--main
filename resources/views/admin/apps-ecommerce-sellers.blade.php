                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="flex items-center gap-2 mb-4">
                                    ...
                                    <div class="relative dropdown shrink-0">
                                        ...
                                        <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="sellersAction1">
                                            <li>
                                                <a data-modal-target="addUpdateModal" data-user="{{ $store->user }}" data-store-info="{{ json_encode($store) }}" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="#!"><i data-lucide="file-edit" class="inline-block mr-1 size-3"></i> <span class="align-middle">Edit</span></a>
                                            </li>
                                            <li>
                                                <a data-modal-target="deleteModal" data-this-id="{{ $store->id }}" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="#!"><i data-lucide="trash-2" class="inline-block mr-1 size-3"></i> <span class="align-middle">Delete</span></a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="flex items-center justify-center mx-auto rounded-full size-16 bg-slate-100 outline outline-slate-100 outline-offset-1 dark:bg-zink-600 dark:outline-zink-600">
                                    <img src="{{ asset('images/adwords.png') }}" alt="" class="h-10 rounded-full">
                                </div>

                                <div class="mt-4 mb-6 text-center">
                                    <h6 class="text-16"><a href="#!">{{ $store->name }}</a></h6>
                                    <p class="text-slate-500 dark:text-zink-200">{{ $store->user->name }}</p>
                                </div>
                            </div>
                        </div><!--end card-->
                    </div><!--end col-->
                ...
                <div class="flex flex-col items-center mb-5 md:flex-row">
                    ...
                    <div class="mb-4 grow md:mb-0">
                        <p class="text-slate-500 dark:text-zink-200">
                            Showing <b>{{ $stores->count() }}</b> of <b>{{ $stores->total() }}</b> Results
                        </p>
                    </div>

                    {{ $stores->links('vendor.pagination.custom-pagination') }}
                </div>
...
<div id="addUpdateModal" modal-center class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
            <h5 class="text-16">Edit Seller</h5>
            <button data-modal-close="addUpdateModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x" class="size-5"></i></button>
        </div>
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form action="{{ route('store_update') }}" method="POST">
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <button type="reset" data-modal-close="addUpdateModal" class="text-red-500 bg-white btn hover:text-red-500 hover:bg-red-100 focus:text-red-500 focus:bg-red-100 active:text-red-500 active:bg-red-100 dark:bg-zink-600 dark:hover:bg-red-500/10 dark:focus:bg-red-500/10 dark:active:bg-red-500/10">Cancel</button>
                    <button type="submit" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">Edit Seller</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="w-screen md:w-[25rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto px-6 py-8">
            ...
            <div class="mt-5 text-center">
                <form action="{{ route('store_delete') }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <input type="hidden" name="this_id">
...
<script src="{{ asset('js/pages/apps-ecommerce-sellers.init.js') }}"></script>
@endsection
